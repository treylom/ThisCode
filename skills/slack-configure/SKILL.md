...
### 🔴 관문 계약 (R2 — 2026-08-13 운영 검토 「'직접 해주셔야 할 것'을 띄우기 전에 반드시 한 번 확인」)

「사람이 해야 한다」는 판단을 **문서가 아니라 명령이 답한다.** 사용자에게 무언가를 부탁하려는 «모든» 지점에서, 부탁하기 **전에** 이 명령을 실제로 부른다:

```bash
bash <플러그인루트>/scripts/install-gate.sh <관문이름>
```

| 관문 | 이 스킬의 단계 | 계약상 이름 | 게이트 답 | 왜 |
|---|---|---|---|---|
| A | Step 3 — Slack CLI 로그인 | `slack_login` | exit 0 | 계정 자격증명 — 대리 입력 금지 영역 |
| B | Step 5 — 앱 생성 | `slack_app_create` | exit 0 | 사용자 계정 소유권으로 앱을 만드는 행위 |
| B | Step 5 — 워크스페이스 설치 승인 | `slack_manifest_approve` | exit 0 | 권한 범위 승인 — 사용자가 읽고 동의해야 함 |
| C | Step 6′ — 메시지 탭 켜기 | `slack_messages_tab_toggle` | exit 0 | 앱 설정 UI 토글 — 소유자 계정 세션에서만 |
| D | Step 10 — 토큰 붙여넣기 | `slack_token_paste` | exit 0 | 시크릿 값 취급 — 사용자가 직접 |
| E | Step 14 — 개발 채널 로드 승인 | `slack_dev_channels_confirm` | exit 0 | 권한 승인 다이얼로그 — 대신 눌러선 안 된다 |

**이 여섯만 exit 0 이다.** 여기 없는 이름으로 사람에게 넘기려 하면 게이트가 `exit 1` 로 막는다 — 그건 「먼저 자동으로 한 번 실제로 시도하라」는 뜻이다. **「시도」는 «판단»이 아니라 «실행»이다**: 도구를 실제로 호출해 보고 그 결과를 본다. "이건 사람이 해야 할 것 같다"는 시도가 아니라 이 계약이 생기기 전과 똑같은 동작이다.

시도했으면 **남긴다** — 성공·실패 둘 다:

```bash
bash <플러그인루트>/scripts/install-gate.sh --attempted <관문이름> ok
bash <플러그인루트>/scripts/install-gate.sh --attempted <관문이름> fail "<무엇이 막았는지>"
```

실패에 사유를 비우면 게이트가 **기록을 거부**한다(exit 2). 확인할 수 없는 계약은 지켜지지 않아도 표가 안 나기 때문이다. 되감아 보려면 `install-gate.sh --audit`.

> **주의 — 위 표의 exit 0 은 「이 관문에서 멈춰도 된다」는 뜻이지 「이 관문 전체가 사람 몫」이라는 뜻이 아니다.** 예: 관문 B 의 매니페스트 «작성·갱신»은 `apps.manifest.update` 로 자동이고(Step 5 의 정정 부기), 사람 몫은 설치 승인 클릭뿐이다. 관문 이름은 **멈추는 그 순간**에 붙는다.

## create-bot과 공유하는 부분

Step 0(USER-PROFILE 인터뷰)·페르소나 템플릿 선택 UX(Step 5)·시그니처 코퍼스 수집(Step 5-A)은 [create-bot](../create-bot/SKILL.md)과 **동일 로직**이라 그대로 재사용한다.

🔴 **WD/CLAUDE.md 생성은 재사용하지 않는다(2026-08-06 정정 — ㉡ 인격·거버넌스 이식)**: create-bot Step 6의 CLAUDE.md 생성은 2026-08-12 부로 Discord 판도 **필수 승격**됐지만(bot.yaml `wd_docs` 기록 — 구 «선택» 라벨은 폐기), 여기서의 논점은 생성 여부가 아니라 **주입 경로**다: Discord 판의 페르소나 주입은 `DISCORD_STATE_DIR` 기반 SessionStart 훅(`bot-session-init.sh`)에 의존한다. 그 훅은 Slack 상태 디렉토리(`CLAUDE_CHANNEL_SLACK_DIR`)를 인식하는 조건이 없어 — 인자도 `DISCORD_STATE_DIR`도 없으면 **무음 종료**(`exit 0`)한다 — Slack 세션에서는 파일만 있고 페르소나가 한 줄도 안 들어간다. 그래서 Slack 판은 WD를 **필수**로 만들고(D-2 확정 — 2026-08-06 결정, `04-handoff.md` §2), 페르소나를 훅이 아니라 **CLAUDE.md 본문에 직접** 박아 넣는 **Step 7-A**를 아래에 새로 정의한다 — cwd 체인로드는 훅 등록 여부와 무관하게 항상 동작하기 때문이다.
...
> codex 짝 = ThisCodex `skills/slack-bridge/SKILL.md` 0단계 (**설치 단계 한정** 교차 계약 — 설치 공정·PATH 함정·실측 라벨·한계는 양쪽 동시 반영. 문서 전체 범위의 교차 계약은 이 계약이 아니라 각 `slack-bridge/SKILL.md:8` 의 것이다).

⚠️ **설치가 성공해도 그 셸에서 바로 안 잡힐 수 있다** — 공식 문서 자인이었던 이 함정은 2026-08-08 클린룸 실측으로 재현 확인됐다(WSL2 · Ubuntu 24.04.1 · x86_64 · v4.6.0 — 설치 5초, 직후 새 셸에서 `slack` = not found. 설치기는 Required manual setup 으로 PATH 등록을 사람에게 미룬다). 그래서 재확인 + PATH 보정 + **영구 등록까지가 이 자동 단계다**(2026-08-08 운영 결정 "slack cli 는 알아서 설치하도록 동봉" — 사람에게 남기던 등록 안내를 자동화로 승격). 블록 E2E 검증 급(2026-08-08 클린룸 WSL): 프로필 전무·전형 우분투 두 시나리오 각 2회 — 완주 exit 0·재다운로드 0·새 셸 `-lc`/`-ic` 모두 잡힘:
...
이 단계는 아래 매니페스트 JSON 을 `<앱 이름>`·`<봇 표시 이름>` 만 Step 0-A 값으로 치환해 **파일로 떨군다**(예: `$HOME/.claude/channels/.slack-configure-manifest.json` — 비밀값 없음, [slack-bridge/SKILL.md](../slack-bridge/SKILL.md) 예시와 동일한 형식):
...
**진입 시 선택 질문**(운영 설계 지시 2026-08-06 — "브라우저 자동화로 하게 하거나, 직접 하게 하거나 선택지를 주는 방향"):
...
🔴 **한 번에 안내 원칙** (2026-08-06 운영 지시 — "스킬 사용 시 한 번에 안내가 되어야 하는데 말이죠"): 위에서 출력하는 매니페스트가 **최종 완결본**이다 — scope·이벤트 구독·설정 전부 포함이며, 이후 어느 단계에서도 권한·설정을 추가로 요구하지 않아야 정상이다. 그래서 3번의 승인 화면 «5줄 확인»이 왕복을 선제로 없애는 자리다(라이브 실측 2026-08-06 결함 15: 선언은 온전했는데 **갱신 후 재설치가 빠져** 부여가 옛것 그대로였고, DM 이 무징후로 죽었다 — 매니페스트 갱신 ≠ 재설치).

**매니페스트 «갱신» 경로 (앱이 이미 있을 때 — 결함 16 처방)**: 이미 만든 앱의 scope·이벤트를 바꿀 때는 브라우저 UI 안내를 다시 밟게 하지 않는다 — 한국어 화면("범위"=Scopes)·스크롤·"이미 있는 항목은 추가 목록에 안 뜸" 함정이 전부 그 화면에 산다(2026-08-06 운영 라이브 검토). 대신:
...
> **D-2 결정**: Slack 응답 세션의 cwd는 수강생 워크스페이스(vault) 안 **봇 전용 루트 폴더**여야 하고, 그 폴더엔 반드시 자기 `CLAUDE.md`가 있어야 한다. 없으면 세션 기동 시 정체성 fallback이 워크스페이스 루트(그 위) CLAUDE.md의 봇 메타로 해소된다. Step 0-A에서 받은 `$BOT_NAME`·`$SOUL_TYPE`·`$ROLE_DESC`를 여기서 실제 파일로 굽는다.
...
0. 🔴 **조사 먼저 — 즉석 창작 금지** (2026-08-06 운영 지시): 자리를 채우기 **전에** 실제 사용자 맥락을 수집한다 — ⓐ워크스페이스에 vault·노트가 있으면 `/km:search`·grep으로 어휘·업무 영역·반복 작업을 훑고 ⓑ기존 봇 `soul.md`가 있으면 어휘·경계 규약을 대조하며 ⓒ둘 다 없으면 사용자에게 주 업무·말투·금지사항을 짧게 인터뷰한다. `$BOT_NAME`·`$ROLE_DESC`만 보고 내용을 지어내면 일반 비서 톤으로 수렴하므로, 확인한 맥락만 사용한다.
1. **잔여 전수 나열**: `command grep -n '<[^>]*>' "$BOT_DIR/soul.md"`
2. **각 자리를 0번 조사 결과를 근거로 채운다**(Edit 도구) — `$BOT_NAME`·`$ROLE_DESC`·`$SOUL_TYPE`에 맞춰 페르소나 어휘·시그니처·경계·출력 규약을 채운다. 시그니처 이모지는 조사 결과에 맞게 정하고, 실측할 수 없는 ID는 `(Step 15-A에서 실측 기입)`으로 남긴다.
3. **Slack 맥락화**: 템플릿은 Discord 판 공용이라 Discord 전용 문구가 남는다 — "Discord DM/mention 수신 시" → Slack 등가로, `mcp__plugin_discord_discord__reply` 도구 지시 → "Slack 응답은 `slack-channel` MCP reply 도구로만" 으로 치환. **Slack 봇에게 Discord 도구를 시키는 문장이 남으면 조용한 오작동을 유발한다**(루돌프 실측: 생성된 soul.md L83). 🔴 **같은 자리에 DM thread_ts 캐치 1줄을 덧붙인다**(2026-08-10, 루돌프 실측 — DM 1:1 답장이 댓글 스레드로 접히는 결함. 코드 레이어 방어는 `claude-channel-server/src/server.ts` handleReply에 별도로 있지만, 모델이 규칙을 모르면 애초에 `thread_ts`를 넘기는 시도 자체가 반복된다): "DM(1:1)에서는 `reply` 도구에 `thread_ts`를 넘기지 않는다 — 받은 메시지의 ts를 되돌려 넣으면 1:1 대화가 댓글 스레드로 접힌다. 인바운드 `<channel>` 태그에 `thread_ts`가 실제로 실려 있을 때만 echo한다(사용자가 스스로 DM 안에서 스레드를 연 경우). 채널 응답은 스레드가 기본이며 브리지가 채운다 — 이 규칙은 DM 한정."
4. **기계 게이트**:
...
> 원본 페르소나 사본 = 같은 폴더 `soul.md`(향후 Slack용 SessionStart 훅이 생기면 그쪽으로 전환 가능 — 지금은 이 CLAUDE.md 본문을 기준으로 사용).
...
2. 기존 채널 이름을 줬으면 **사용자에게 직접 확인을 요청하는 것이 기본 경로다** — 단, 안내는 UI 위치가 아니라 **「링크 복사」 1순위**로 준다 (2026-08-06 라이브 검토에서 UI 위치 안내보다 링크 복사가 안정적이었다):
...
**봇 채널 초대 안내 — 초대 명령은 «영문 핸들» 기준으로 박아서 준다** (2026-08-06 라이브 검토에서 한글 표시명보다 영문 핸들이 안정적으로 검색됨): 안내문에는 항상 되는 경로인 `bot_user.display_name`(영문) 실제 값을 채워 출력한다:
...
> **왜 여기인가** (2026-08-06 결함 15 라이브 실측 + 운영 지시): **매니페스트 갱신 ≠ 재설치.** Slack은 scope가 늘면 사람이 다시 승인(재설치)해야 실부여된다 — 앱 설정 화면에는 권한이 다 들어가 있어도 OAuth 승인이 옛것이면 토큰 부여는 옛것 그대로다. `im:history` 미부여는 `message.im` 자체가 오지 않아 오류·경고 없이 DM이 멈추는 원인이므로, 토큰을 받은 직후 선언과 부여를 대조한다.
...
- **기본이 bypass 인 이유**(운영 지시 + 실측 근거): 승인 대기로 멈춘 봇은 Slack 쪽에서 **무응답과 구별되지 않는다** — 결함 14(승인 프롬프트 누출)의 반대편 실패 모드(누출 vs 조용한 정지)를 둘 다 닫으려면 상시 봇은 bypass가 기본이어야 한다. permission-relay(`handlePermissionAsk`·verdict 경로)는 `-safe` 변형이 쓴다.
...
6-b. **부여 scope 재확인 — [Step 10-c](#step-10-c-부여-scope-관문-자동--부족하면-여기서-정지-다음-단계로-넘어가지-않는다) 와 같은 검사를 그대로 재실행한다**(기준 블록은 10-c 하나 — 사본을 두지 않는다). 설치 후 사용자가 앱 설정을 바꿨거나 재설치로 토큰이 회전된 경우를 여기서 다시 잡는다.
...
- **①에서 다른 봇 이름(예: 워크스페이스 상위 CLAUDE.md의 봇 메타)이 나오면** = cwd가 봇 루트 폴더가 아니었다는 뜻이다 — Step 13-A/14를 다시 확인한다. `04-handoff.md` §3.4가 실측한 것과 동일한 증상(CLAUDE.md 없는 폴더에서 기동 → 정체성이 상위로 fallback)이다.
...
8. **SessionStart 훅(`bot-session-init.sh`) 확장은 이번 패치 범위 밖이다(의도적)** — Slack 상태 디렉토리를 훅이 인식하도록 고치는 경로(D-2′)는 운영 결정 대기 항목이라, 이번 Step 7-A는 훅에 의존하지 않는 CLAUDE.md 직접 임베드로 우회했다. 훅 확장이 나중에 결정되면 이미 만들어 둔 `soul.md` 사본을 그 경로가 대신 읽도록 전환할 수 있다 — 지금 상태에서 재작업이 크지 않다.
...
- Discord 판(공유 로직의 기준 문서): [../create-bot/SKILL.md](../create-bot/SKILL.md)
