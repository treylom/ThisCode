...
| 파라미터 | 기본값 | 오버라이드 키워드 |
|---------|--------|-----------------|
| 대상 폴더 | ${ARGUMENTS}에서 경로 추출 | 명시적 폴더 경로 |
...
${ARGUMENTS}에서 아래 키워드를 순서대로 매칭합니다. **첫 매칭이 적용됩니다.**
...
|--------|---|
| "단일", "하나로", "single" | 단일 노트 |
| "분할", "나눠", "주제별", "2티어", "2-tier", "split" | 주제별 분할 (2-tier) |
| "원자", "zettelkasten", "atomic" | 원자적 분할 |
| "3-tier", "계층", "tier" | 3-tier 계층 |
| (매칭 없음 + 요약) | **단일 노트** |
| (매칭 없음 + 보통) | **3-tier 계층** (기본화 — 명시 선택은 항상 우선) |
| (매칭 없음 + 상세) | **3-tier 계층** |
