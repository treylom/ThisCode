...
- If you stop because an approval is legitimately pending, end the last message with an explicit waiting verdict — e.g. `verdict: waiting for approval — <reason>` — so a stop gate or a reviewer can tell a sanctioned pause from an unfinished task.
