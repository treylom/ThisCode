...
| Delegating to / waiting on a bot · asserting a bot's identity/health · multi-agent coordination · handing someone a duration estimate · deciding whether to distribute/delegate at task kickoff | [orchestration.md](orchestration.md) | Verify bot identity (chain-load guard), don't assume; teammate idle = drive not wait; meeting = adopt domain frames, no solo lock; **five delegation defaults at kickoff (§9): 3+ docs = distribute, 3+ stages = workflow-managed, specialty match = delegate, orchestrator overload = hand off, search-before-write** |
