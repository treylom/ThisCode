...
- **A teammate's remaining-context % is NOT grounds for pausing, excluding, or restarting it.** Modern harnesses auto-compact on their own (Claude Code natively; Codex CLI likewise — for codex-family bots, ignore the gauge entirely). Default = keep pushing; compaction is each session harness's job.
