...
// ─────────────────────────────────────────────── R1: 「가장 먼저」는 순서다
// start.md 의 Step 0-A 는 「다른 어떤 것보다 앞에」라고 «적혀» 있다. 적힌 순서는
// 누가 위에 절을 하나 끼워도 안 죽는다 — 문장은 그대로고 순서만 바뀐다.
// 그래서 «적힌 말»이 아니라 «실제 위치»를 본다.

test('R1 — 설치 방식 질문(Step 0-A)이 start.md 의 첫 번째 절이다', () => {
  const lines = readFileSync(join(REPO, 'commands', 'start.md'), 'utf8').split('\n');
  const headings = lines
    .map((l, i) => ({ i, l }))
    .filter(({ l }) => /^##\s/.test(l));
  // 기대값 게이트: 제목이 하나도 안 잡히면 아래 비교가 «검사 없이» 통과한다.
  assert.ok(headings.length >= 3, `## 제목을 ${headings.length}개만 찾았다 — 문서 구조가 바뀌었다`);
  assert.match(headings[0].l, /Step 0-A/,
    `첫 번째 절이 Step 0-A 가 아니라 "${headings[0].l.trim()}" 다 — R1(가장 먼저 묻는다)이 깨졌다`);
});
