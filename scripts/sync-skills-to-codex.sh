...
# Expose ThisCode skills to a Codex CLI agent.
#
...
# Skills to sync. 1.4.0: the knowledge/search family moved out of this repo, so
# the list is empty; add skill directory names here to resume syncing. Keep in
# step with scripts/lib/codexsync.mjs (KM_SKILLS).
SKILLS=()

# Codex compatibility (allowedTools-derived): anything not explicitly classified.
compat() {
  case "$1" in
    *) echo "unknown" ;;
  esac
...
echo ""
if [ "$MODE" = "--check" ]; then
  echo "[sync] dry run only. Re-run with --apply to copy."
else
  echo "[sync] done. Verify: start a Codex session / 'codex exec' and check the"
  echo "       active skill list includes the synced skills."
fi
exit $fail
