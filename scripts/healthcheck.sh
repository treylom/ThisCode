...
phase_check "Phase 1" "ripgrep (local text)"             "command -v rg || command -v grep" "required"
phase_check "Phase 2" "obsidian-cli (local tool)"        "command -v obsidian-cli || command -v obsidian || command -v notesmd-cli" "optional"
phase_check "Phase 3" "vault-search MCP (local embedding)" "claude mcp get vault-search 2>/dev/null || jq -e '.mcpServers.\"vault-search\"' ${HOME}/.config/claude/claude_desktop_config.json 2>/dev/null" "optional"
phase_check "Phase 4" "GraphRAG (local server)"          "curl -fsS http://localhost:8400/health 2>/dev/null" "optional"
