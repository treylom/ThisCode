...
phase_check "Phase 2" "obsidian-cli (Tier 2)"         "command -v obsidian-cli || command -v obsidian || command -v notesmd-cli" "optional"
phase_check "Phase 3" "vault-search MCP (Tier 3)"     "claude mcp get vault-search 2>/dev/null || jq -e '.mcpServers.\"vault-search\"' ${HOME}/.config/claude/claude_desktop_config.json 2>/dev/null" "optional"
