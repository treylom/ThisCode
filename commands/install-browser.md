...
실패하면 중단하고 카드 C를 안내합니다. 사용자 전체 설정으로 바꾸지 않습니다. 같은 이름의 연결이 사용자 전체 범위에도 있으면 카드 F를 안내합니다.
...
실패하면 카드 E를 안내합니다. 모델의 “성공했습니다” 문장이 아니라 페이지 열기·스냅샷·제목의 기계 이벤트 3개와 일반 프로젝트의 Playwright 연결 승인 상태로 판정합니다. 승인 대기 상태면 자동으로 우회하지 않고, 일반 Claude Code 세션에서 한 번 승인한 뒤 같은 명령을 한 번만 재실행합니다. Playwright 연결이 사용자 전체 범위에도 등록되어 있다는 안내가 나오면 카드 F를 안내합니다.
