...
**그리고 시도했으면 «남긴다»** — exit 1/2 를 받고 시도한 뒤에는 반드시:
...
실패인데 사유를 비우면 게이트가 **기록을 거부한다**(exit 2). 사유 없는 수동 낙하는 계약 위반이기 때문이다.

기록이 필요한 이유는 감시가 아니라 **판정 가능성**이다. 이 계약의 합격 조건이 「시도 1회 + 그 결과 기록」인데 기록이 없으면, 계약을 지켰는지 아무도 — 나중의 나를 포함해 — 확인할 수 없다. 되감아 보려면:

```bash
bash <플러그인루트>/scripts/install-gate.sh --audit
```

이 감사가 답하는 질문은 하나다: **「시도하라는 판정을 받고도 시도 기록이 없는 관문은 무엇인가」.** 수동 안내가 화면에 실제로 떴는지는 답하지 «않는다» — 게이트는 화면을 못 본다. 그러므로 위반 0 은 「계약 준수」가 아니라 「미이행 흔적 없음」으로 읽는다.
